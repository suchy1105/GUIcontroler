...
RUN printf '%s\n' 'path-exclude /usr/share/doc/*' 'path-include /usr/share/doc/*/copyright' 'path-exclude /usr/share/man/*' 'path-exclude /usr/share/groff/*' 'path-exclude /usr/share/info/*' 'path-exclude /usr/share/lintian/*' 'path-exclude /usr/share/linda/*' > /etc/dpkg/dpkg.cfg.d/01_nodoc && \
    ...
    apt-get update && apt-get install --no-install-recommends -yq tzdata ca-certificates fontconfig ttf-mscorefonts-installer imagemagick x11-xserver-utils apt-utils libegl1 libegl1-mesa libegl-mesa0 make golang && \
...
RUN make /app/
#RUN go mod tidy
RUN go get github.com/suchy1105/GUIcontroler
